...
Trente lots commerciaux ont été échantillonnés pour valider la performance du modèle. Une propriété dont l'utilisation est commerciale a une erreur de plus de 300 places. Cette propriété est un immeuble commercial construit dans les années 1970 et se trouve dans la zone industrielle de la Cité-Universitaire.  Un filtre pour les erreurs de plus 75 places a été appliqué pour l'enlever pour le calcul des statistiques. Ce filtrage change la moyenne de la catégorie de $\bar{y}_{pred} = 55.6 $ à $\bar{y}_{pred} = 46.3 $.  La valeur moyenne observée $\bar{y}_{obs}$ est 44.0.\par
