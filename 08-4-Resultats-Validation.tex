...
\end{tabular}
\end{table}
En plus des statistiques agrégées, la section suivante montrera neuf figures, présentées dans le même ordre. La première figure  montre la relation entre la valeur prédite et la valeur observée (a) et la distribution des prédictions pour l'échantillon d'évaluation comparé à l'ensemble des prédictions (b). La deuxième montre les erreurs (a) et les erreurs au carré (b) en fonction de la valeur prédite (c). La troisième figure montre la distribution des erreurs (b) et un diagramme QQ (a) comparant la distribution d'erreur à une distribution normale. La quatrième montre la distribution de l'erreur moyenne (a) et l'erreur absolue moyenne (b) lorsque l'on applique une méthode d'autoamorçage pour donner un aperçu de la dispersion des métriques d'erreur si un échantillon légèrement différent était sélectionné. Ces quatre figures sont filtrées si un critère a été trouvé. \par
Les cinq derniers graphiques montrent  les erreurs en fonctions de variables qui sont jugées pertinentes pour voir quels pourraient être les facteurs explicatifs des erreurs. Ces graphiques seront potentiellement répliqués si un filtrage doit être appliqué pour éliminer des données aberrantes qui rendent invisibles le reste des données. D'autre part, lorsque le nombre de places de stationnement est bas, du bruit aléatoire est parfois ajouté à certains graphiques XY pour mieux communiquer la distribution des points. Les statistiques d'intervalles de prédictions et de valeurs moyennes sont calculées à l'aide des valeurs non bruitées. D'autre part, la distance au parlement à vol d'oiseau a été calculée pour donner une mesure de dispersion géographique pour l'analyse des résidus. La cinquième figure montre les erreurs en fonction de la superficie d'étage et le nombre de logements. La sixième montre l'erreur en fonction de l'année de construction et de la distance au parlement. La septième montre l'erreur de prédiction en fonction de l'utilisation d'unités converties et de la valeur prédite. La huitième montre l'erreur en fonction de la superficie du lot et de la valeur au rôle. La dernière figure montre l'erreur en fonction à la densité foncière (graphique (a), eq. \ref{eq:lot-tax-density}) et son rapport au stationnement prédit (graphique (b), eq. \ref{eq:intensite-stat-pred}).\par
