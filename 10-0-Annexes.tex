...
\Annexe{Méthodes statistiques recensées}\label{ann:meth-stat-recens}
    Un cadre de quantification de l'incertitude utilisant des méthodes basées sur la statistique inférentielle ont été considérées pour l'évaluation de la performance de modèle. Cependant, un cadre plus basé sur la statistique prédictive a été adopté ou des tailles d'effets n'étaient pas immédiatement apparents et ont donc été conservés en annexe. Cette annexe contient le résultat de cette revue de littérature. Elle commencera par discuter des méthodes d'équivalence initialement utilisées pour déclarer deux médicaments assez proches, puis discutera des enjeux entourant les valeurs p communément utilisées statuer sur la validité de constats scientifiques et finalement des différentes caractéristiques qui peuvent être utilisées pour statuer sur la forme d'une distribution.
...
Tel que mentionné à la section \ref{ssec:data-geopolitique-territoire}, une légère modification du découpage municipal brut d'Élections Québec \parencite{elections_quebec_atlas_2021} a été faite pour maintenir une différentiation entre le zonage de la vieille partie de Québec et le secteur des Rivières.
