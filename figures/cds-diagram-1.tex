...
\begin{document}
    \begin{tikzpicture}
        ...
        % Métrique
        \node (sessions) [objet,below of=status,yshift=-1.25cm]{Sessions};
        \node (aggregates) [objet, left of=sessions, xshift=-2.5cm]{Statistiques agrégées};
        \draw (sessions.west) -- (aggregates.east);
        \node (total_session) [objet, left of=aggregates,xshift=-3cm,yshift=0.4cm]{Sessions totales};
        ...
        \node (dwell_time) [objet, left of=aggregates,xshift=-3cm,yshift=-0.4cm]{Durée sessions};
        ...
        % Légende
        \node (legend) [below of=sessions, text width=3cm,xshift=1cm,yshift=-0.75cm]{Légende};
        \coordinate (boite_top_left)at($(legend.north west)+ (-0.2,0.2)$);
        \node (objet_desc)[objet,below of=legend,yshift=0.3cm]{Objet};
