...
\newcommand{\annexsection}[1]{%
  ...
  \section*{#1}% print the title
  \addcontentsline{app}{section}{\protect\numberline{\thesection}#1}% only to annex ToC
}
...
% Nom et titre
\renewcommand{\lstlistingname}{Extrait de code}          % pour le caption dans le texte
\renewcommand{\lstlistlistingname}{LISTE DES EXTRAITS DE CODE} % pour la liste
\makeatletter
\renewcommand*{\l@lstlisting}[2]{%
  \@dottedtocline{1}{1.5em}{3em}{\lstlistingname~#1}{#2}%
}
\makeatother

% réduction du nombre d'alarmes
\usepackage{silence}
...
% Apparence caption tbale figure
...
% Commandes pour divers petits modif visuelles
...
\begingroup
\let\oldaddcontentsline\addcontentsline % on sauvegarde
\renewcommand{\addcontentsline}[3]{}   % on bloque l'ajout d'entrées dans la ToC
\tableofcontents                       % La ToC s'affiche, mais aucune entrée n'est créée
\let\addcontentsline\oldaddcontentsline % on restaure
\endgroup
%%
%% Liste des tableaux
%% List of tables
...
\phantomsection
    ...
    % Titre centré manuel
    %\begin{center}
    %\textbf{LISTE DES EXTRAITS DE CODE}
    %\end{center}
    %\hspace{0.5em} 
    % Affiche la liste brute, sans saut de page
    
    \lstlistoflistings


% List des sigles et abbrev
