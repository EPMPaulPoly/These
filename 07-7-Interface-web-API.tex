...
\end{itemize}
Les détails d'implémentation sont discutés dans la section des résultats d'outils à la section \ref{ssec:outil_develope}.
\subsection{Requis}
...
\paragraph{Capacité de créer un échantillon de validation} L'outil doit permettre de créer une stratégie d’échantillonnage selon les paramètres de l'utilisateur et recenser le stationnement pour le comparer à la valeur prédite et inférer si la prédiction du modèle est statistiquement valide.
...
\section{Validation du recensement}
Le calcul de l'inventaire à partir des données du rôle foncier constitue un modèle de la réalité où le nombre de places requises a été construit exactement. Bien que la littérature scientifique supporte l'idée que ces requis sont un   